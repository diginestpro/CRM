-- ============================================
-- STEP 15: Complete database fix script
...
-- 3. Add branding fields to companies
ALTER TABLE public.companies
  ...
  ADD COLUMN IF NOT EXISTS brand_color TEXT DEFAULT E''#2563eb'',
  ADD COLUMN IF NOT EXISTS footer_text TEXT,
  ADD COLUMN IF NOT EXISTS invoice_template TEXT DEFAULT E''modern'';

-- 4. company_settings table (no unique constraint here - handled separately)
...
-- 4b. Add unique constraint with quoted column name
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = ''company_settings_company_name_unique''
  ) THEN
    EXECUTE ''ALTER TABLE public.company_settings ADD CONSTRAINT company_settings_company_name_unique UNIQUE (company_id, "key" text)'';
  END IF;
END $$;
...
-- 7. Make sure company_users RLS is correct
