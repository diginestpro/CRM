-- =============================================================================
-- SEED TEST DATA - Run this in Supabase SQL Editor
-- =============================================================================
...
BEGIN
  -- Get or check company
  SELECT id INTO v_company_id FROM public.companies LIMIT 1;
  IF v_company_id IS NULL THEN
    RAISE NOTICE 'NO COMPANY FOUND - Please complete onboarding at /onboarding first';
    RETURN;
  END IF;
  RAISE NOTICE 'Using company: %', v_company_id;

  -- Seed currencies (only if empty)
  ...
  ELSE
    RAISE NOTICE 'Currencies already exist (%)', v_existing_count;
  END IF;
  ...
  ELSE
    RAISE NOTICE 'Services already exist (%)', v_existing_count;
  END IF;
  ...
  ELSE
    RAISE NOTICE 'Clients already exist (%)', v_existing_count;
  END IF;
  ...
  ELSE
    RAISE NOTICE 'Quotations already exist (%)', v_existing_count;
  END IF;
  ...
  ELSE
    RAISE NOTICE 'Invoices already exist (%)', v_existing_count;
  END IF;
  ...
  IF v_existing_count = 0 THEN
    ...
    END IF;
  ELSE
    RAISE NOTICE 'Payments already exist (%)', v_existing_count;
  END IF;

  RAISE NOTICE '=== SEED COMPLETE - Check dashboard now! ===';
END $$;
