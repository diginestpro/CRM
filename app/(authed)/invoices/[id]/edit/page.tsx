  ...
  async function onSubmit(v: FormValues) {
    ...
    try {
      ...
      const { error: itemE } = await sb.from("invoice_items").insert(v.items.map(i => ({
        invoice_id: id, service_id: i.service_id, quantity: i.quantity, unit_price: i.unit_price, description: i.description,
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-end gap-3">
          <Button variant="outline" asChild><Link href={`/invoices/${id}`}>Cancel</Link></Button>
